...
#define FileName "News-Navigator"
...
public:
   ...
   int               init()
   {
      ...
      if(vip.CheckGroup(Permissions2, false))
      {
         ...
         api.key = apikey;
         api.url = "https://www.jblanked.com/news/news-navigator/";
         ...
         {
            if(api.GET(EasyButtonAPIPage, vip.headers.toStr(), "https://www.jblanked.com/", true))
            {
               if(!this.isTester())
               {
   ...
   void              onChartDownload(const int id, const long &lparam, const double &dparam, const string &sparam)
   {
      ...
      if(id == CHARTEVENT_OBJECT_CLICK)
      {
         ...
         {
            Alert("Downloading News-Navigator now...");
            if(!d.downloadAndExtract("https://www.jblanked.com/trading-tools/download/tools/News-Navigator.zip",
                                     "News-Navigator.zip"))
            {
               Print("Failed to download.. Trying again...");
               d.downloadAndExtract("https://www.jblanked.com/trading-tools/download/tools/News-Navigator.zip",
                                    "News-Navigator.zip");
            }
      ...
      if(!this.isTester())
      {
         ...
         if(!this.isTester())
         {
            ...
            if(current_3rd > current_5th && current_3rd != last_time)
            {
               ...
               bool got = api.GET(EasyButtonAPIPage, vip.headers.toStr(), "https://www.jblanked.com/", false);
               if(!got)
               {
                  int iterr = 0;
                  do
                  {
                     Sleep(1000);
                     got = api.GET(EasyButtonAPIPage, vip.headers.toStr(), "https://www.jblanked.com/", true);
                     iterr++;
                  }
                  while(!got && iterr < 10);
               }
